...
def take_step(pos: Dict[str, int], grid: List[List[str]]) -> Dict[str, Any]:
    ...
    new_pos = {"x": new_x, "y": new_y, "h": new_heading, "t": new_tile, "counter": counter + 1}
    if new_heading == "HOME":
        return new_pos
...
def take_first_step(start: Dict[str, int], grid: List[str]) -> List[List[str]]:
    ...
    try:
        ...
        east_pos = {"x": x + 1, "y": y, "h": "E", "t": next_tile, "counter": 1}
        if next_tile in ["-", "J", "7"]:
    ...
    try:
        next_tile = grid[y][x-1]
        west_pos = {"x": x - 1, "y": y, "h": "W", "t": next_tile, "counter": 1}
        if next_tile in ["-", "F", "L"]:
    ...
    try:
        ...
        north_pos = {"x": x, "y": y - 1, "h": "N", "t": next_tile, "counter": 1}
        if next_tile in ["|", "7", "F"]:
    ...
    try:
        next_tile = grid[y + 1][x]
        south_pos = {"x": x, "y": y + 1, "h": "S", "t": next_tile, "counter": 1}
        if next_tile in ["|", "L", "J"]:
