...
class Brick:
    ...
    def get_bricks_under(self, brick_dict: Tuple[Any]):
        under = []
        for u in self.units:
            for z in range(0,u.x):
                coord = (u.x, u.y, z)
    ...
    def __str__(self):
        return f"Brick: [{self.num}] start={self.starts}; end={self.ends}; volume={self.volume} units={self.units}"
...
def solve(input_string: str) -> List[int]:
    ...
    brick_dict = {b.num: b for b in bricks}
    print(f"brick_dict = {brick_dict}")
    occupation_dict = get_occupation_dict(bricks, x_max, y_max, z_max)
